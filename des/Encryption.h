...
public:
	...
	string Code() {
		...
		for (int i = 0; i < 64; i++) {
			if (i < 32)
				L1[i] = M[i];//L1[i] = M[IP[i] - 1];
			else
				L2[i - 32] = M[i]; //L2[i - 32] = M[IP[i] - 1];
		}
